...
import workbench.db.rdb.RdbStatementHook;
...
public class SetCommand
  ...
  public StatementRunnerResult execute(String userSql)
    ...
    if (command != null) {
      ...
      if (command.equalsIgnoreCase("autocommit")) {
        ...
        execSql = !handled;
      } else if (currentConnection.getMetadata().isRdb()) {
        result = new StatementRunnerResult(userSql);
        boolean handled = handleRdb(result, userSql);
        execSql = !handled;
      }
    }
  ...
  private boolean handleRdb(StatementRunnerResult result, String sql) {
    SQLLexer lexer = SQLLexerFactory.createLexer(currentConnection, sql);
    SQLToken token = lexer.getNextToken(false, false);
    ...
    if (token.getText().equalsIgnoreCase("plan")) {
      ...
      boolean on = false;
      if (token == null) {
        on = !runner.getBoolSessionAttribute(RdbStatementHook.SESS_ATTR_SHOWPLAN);
      } else {
        on = token.getText().equalsIgnoreCase("ON");
      }
      runner.setSessionProperty(RdbStatementHook.SESS_ATTR_SHOWPLAN, Boolean.toString(on));
      String flagKey = on ? "TxtOn" : "TxtOff";
      String msg = ResourceMgr.getFormattedString("MsgFbExecPlan", ResourceMgr.getString(flagKey));
      result.addMessage(msg);
      return true;
      ...
      boolean on = false;
      if (token == null) {
        on = !runner.getBoolSessionAttribute(RdbStatementHook.SESS_ATTR_PLAN_ONLY);
      } else {
        on = token.getText().equalsIgnoreCase("ON");
      }

      runner.setSessionProperty(RdbStatementHook.SESS_ATTR_PLAN_ONLY, Boolean.toString(on));
