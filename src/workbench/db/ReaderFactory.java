...
import workbench.db.rdb.RdbConstraintReader;
import workbench.db.rdb.RdbIndexReader;
import workbench.db.rdb.RdbProcedureReader;
import workbench.db.rdb.RdbSequenceReader;
...
public class ReaderFactory {
  public static ProcedureReader getProcedureReader(DbMetadata meta) {
    ...
    if (meta.isRdb()) {
      return new RdbProcedureReader(meta.getWbConnection());
    }
    if (meta.isSqlServer()) {
  ...
  public static SequenceReader getSequenceReader(WbConnection con) {
    ...
    if (meta.isRdb()) {
      return new RdbSequenceReader(con);
    }
  ...
  public static IndexReader getIndexReader(DbMetadata meta) {
    ...
    if (meta.isRdb() && JdbcUtils.hasMinimumServerVersion(meta.getWbConnection(), "2.5")) {
      return new RdbIndexReader(meta);
    }
    if (meta.isMySql()) {
  ...
  public static ConstraintReader getConstraintReader(DbMetadata meta) {
    ...
    if (meta.isRdb()) {
      return new RdbConstraintReader();
    }
